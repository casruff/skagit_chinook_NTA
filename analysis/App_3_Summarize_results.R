...
## ----load_mod_fits-----------------------------------------------------------------------
best_fit <- readRDS(file.path(savedir, "fit_ricker_base.rds"))
...
MC <- 100
set.seed(123)
idx <- sample(seq(5000),MC)


for(i in 1:MC) {
  lines((seq(xM)*exp(aa[idx[i]]-bb[idx[i]]*seq(xM))), col="darkgray")
}
...
for(i in 1:mcmc) {
  # spawners at MSY
  ref_pts[i,"Smsy"] <- (1 - lambert_W0(exp(1-aa[i]))) / beta[i]
  # MSY
  ref_pts[i,"MSY"] <- ref_pts[i,"Smsy"]*((exp(aa[i]-beta[i]*ref_pts[i,"Smsy"])) - 1)
  # harvest rate at MSY
...
hist(Sp_ts[Sp_ts<3e4], breaks = 40,
     col = clr, border = "blue3", 
...
tbl_refpt_smry <- apply(ref_pts,2,quantile,CI_vec)
