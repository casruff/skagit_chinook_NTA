...
fn_harv <- "skagit_chinook_aeq.csv"
```
...
ln_dat_smolt <- c(log(dat_esc$smolt),rep(NA,n_fore))
```
...
model {
  ...
  ## PRIORS
  ##--------
  ## alpha_1 = exp(a) = intrinsic productivity for spawner_smolt transition
  alpha_1 ~ dnorm(0,0.01) T(0,);
  mu_Rkr_a_1 <- log(alpha);
  # E_Rkr_a_1 <- mu_Rkr_a_1 + sigma_r/(2 - 2*phi^2);
  
  ## alpha_2 = exp(a) = intrinsic productivity for smolt_adult transition
  alpha_2 ~ dnorm(0,0.01) T(0,);
  mu_Rkr_a_2 <- log(alpha);
  E_Rkr_a_2 <- mu_Rkr_a_2 + sigma_r/(2 - 2*phi^2);
  
  
  ## strength of dens depend for smolt to adult transition. 
  beta_inv ~ dnorm(0, 1e-9) T(0,);
  beta <- 1/beta_inv;
  ...
  ## AR(1) coef for proc errors for smolt to adult life stage transition
  # phi ~ dunif(-0.999,0.999);
  phi <- 0;
  ...
  ## obs variance for spawners
  tau_spawn <- 1/sigma_spawn;
  sigma_spawn ~ dnorm(0, 0.001) T(0,);
  
  ## obs variance for smolts
  tau_smolt <- 1/sigma_smolt;
  sigma_smolt ~ dnorm(0, 0.001) T(0,);
