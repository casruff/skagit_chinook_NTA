---
title: "Skagit Spring Chinook Spawner Recruit Model and risk assessment procedure: Detailed instructions for replicating analyses and reproducing results."
output:
  pdf_document:
    toc: yes
    toc_depth: '3'
...
We begin by specifying the names of four necessary data files that contain the following information:
 ...
 2. observed age composition of terminal catch + escapement by year;
 3. observed total harvest (exploitation rate) by year;
...
Now we can specify the various models in JAGS. There are 4 models total: 2 different error models  for each of the Ricker and Beverton-Holt forms, with and without AR(1) process errors. 
...
Here is our estimate of the recruitment for each brood year

```{r plot_recruits, fig.width=6, fig.height=4.5, fig.pos="placeHere"}
...
Here is our estimate of the recruits per spawner over time

```{r plot_R_per_S, fig.width=6, fig.height=4.5, fig.pos="placeHere"}
...
Here are probability profiles for the three criteria assessed accross the range of target exploitation rates from 0% - 80%
