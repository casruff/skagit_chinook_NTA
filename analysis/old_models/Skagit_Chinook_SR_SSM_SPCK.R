...
tbl_WAIC <- data.frame(errors=c(2,2,3,3),
 ...
 #         file = "Skagit_SPChin_SR_model_selection_all.csv")
...
R_b_est[R_b_est>3e3] <- 3e3
brks <- seq(Re2prec(min(R_b_est),"floor",2000),3e3,length.out=length(seq(0,9,0.2)))
hist(R_b_est, freq=FALSE, breaks=brks, col=clr, border="blue3",
	 xlab="", xaxt="n", yaxt="n",
	 main="", ylab="", cex.lab=1.2)
axis(1, at=seq(Re2prec(min(R_b_est),"floor",2000),3000,1000))
aHt <- (par()$usr[4]-par()$usr[3])/12
